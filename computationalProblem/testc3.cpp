...
void forSingleDigit(int n,string *str)
{
	const char* forSingleDigit[]={"ZERO ","ONE ","TWO ","THREE ","FOUR ","FIVE ","SIX ","SEVEN ","EIGHT ","NINE ","TEN ","ELEVEN ","TWELVE ","THIRTEEN ","FOURTEEN ","FIFTEEN ","SIXTEEN ","SEVENTEEN ","EIGHTEEN ","NINETEEN "};
	if(n>0 && n<20) 
	{
		*str+=forSingleDigit[n];
	}
}
...
void forDoubleDigit(int n,string *str)
{
	const char* dobuleDigit[]={"","","TWENTY ","THIRTY ","FORTY ","FIFTY ","SIXTY ","SEVENTY ","EIGHTY ","NINETY "};
...
// 		Convert absolute value to words and prepend "MINUS"
	...
	return "[ERROR] : decidePositiveYaNegative.";
}
int main()
{
	//	int n=0;
	long long n=0;
	cout<<"Enter 0 to close the loop\n";
//	while (n != -1)  // if i put - it tale as 0 'ZERO'  i/p -> -  n=0  ya i/p -> 5-  n=5 , n=- -> n=0
	while (true)
	{
		cout<<"Enter the number : ";

		cin>>n;
		if(n==0)        {cout<< "In the words     : ZERO"<<endl; break;}
