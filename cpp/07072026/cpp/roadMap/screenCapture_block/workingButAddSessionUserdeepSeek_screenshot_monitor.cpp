...
// Works on GNOME 42+ Wayland, disables Print Screen keys
...
//working But Add SessionUser() func
//
// # Before running, as root:
// export DISPLAY=:0
// export XAUTHORITY=/home/rushi/.Xauthority
// sudo -E ./screenshot_monitor   # -E preserves the environment
...
static std::string getSessionUser() {
    // Change this to your username or detect dynamically
    return "rushi";
}
...
static void disableGnomeShortcuts() {
    ...
    // List of dconf paths and the value to set (empty array in GVariant format)
    std::vector<std::pair<std::string, std::string>> dconfKeys = {
        {"/org/gnome/shell/keybindings/show-screenshot-ui", "@as []"},
        {"/org/gnome/shell/keybindings/screenshot-window", "@as []"},
        // Legacy keys – may not exist, but safe to try
        {"/org/gnome/shell/keybindings/screenshot", "@as []"},
        {"/org/gnome/settings-daemon/plugins/media-keys/screenshot", "@as []"}
    };

    // Use dconf write directly (with error suppression)
    for (const auto& pair : dconfKeys) {
        std::string cmd = "dconf write " + pair.first + " \"" + pair.second + "\" 2>/dev/null";
        runAsUser(cmd, true);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Also try gsettings as fallback (with error suppression)
    std::vector<std::string> gsettingsCmds = {
        "gsettings set org.gnome.shell.keybindings show-screenshot-ui '[]'",
        "gsettings set org.gnome.shell.keybindings screenshot-window '[]'",
        "gsettings set org.gnome.shell.keybindings screenshot '[]' 2>/dev/null",
        "gsettings set org.gnome.settings-daemon.plugins.media-keys screenshot '[]' 2>/dev/null"
    };
    for (const auto& cmd : gsettingsCmds) {
        runAsUser(cmd, true);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Disable KDE shortcuts (if present)
    std::vector<std::string> kdeCmds = {
...
static void restoreGnomeShortcuts() {
    ...
    // Reset dconf keys
    std::vector<std::string> dconfReset = {
    ...
    for (const auto& key : dconfReset) {
        runAsUser("dconf reset " + key + " 2>/dev/null", true);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Reset gsettings
    std::vector<std::string> gsettingsReset = {
        ...
        "gsettings reset org.gnome.shell.keybindings screenshot 2>/dev/null",
        "gsettings reset org.gnome.settings-daemon.plugins.media-keys screenshot 2>/dev/null"
    };
    for (const auto& cmd : gsettingsReset) {
        runAsUser(cmd, true);
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }

    // Restore KDE shortcuts
    std::vector<std::string> kdeRestore = {
