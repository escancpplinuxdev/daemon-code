...
if grep -Fxq "$DAY" "$HOLIDAY_FILE"; then
    ...
    notify-send -u normal -t 600000 "Holiday Reminder !!!!!" \
    "Tomorrow [$DAY / $TDAY] is a company holiday. Take a leave."

else
    echo "$DAY is a regular work day."

    sudo -u $USER_NAME DISPLAY=$DISPLAY DBUS_SESSION_BUS_ADDRESS=$DBUS_SESSION_BUS_ADDRESS \
    notify-send -u normal -t 600000 "Good morning ....." \
    "Today  [$DAY / $TDAY]  is a regular work day."
fi
